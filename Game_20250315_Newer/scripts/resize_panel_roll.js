// Resize logic for roll panel.

document.addEventListener('DOMContentLoaded', (event) => {
    const rollPanel = document.querySelector('#rollPanel'); // Updated selector to match your ID
    const rollTitle = document.querySelector('.roll-title');
    ...
    // When the user clicks down on the roll title, start dragging
    rollTitle.addEventListener('mousedown', (e) => {
        isDragging = true;
        ...
        initialX = rollPanel.offsetLeft;
        initialY = rollPanel.offsetTop;
    ...
    resizeHandle.addEventListener('mousedown', (e) => {
        isResizing = true;
        initialHeight = rollPanel.clientHeight; // Get the initial height
    ...
    // When the user moves the mouse, move the roll panel if dragging or resize if resizing
    document.addEventListener('mousemove', (e) => {
        if (isDragging) {
            ...
            rollPanel.style.left = `${initialX + dx}px`;
            rollPanel.style.top = `${initialY + dy}px`;
        }
        ...
        if (isResizing) {
            ...
            rollPanel.style.height = `${Math.max(newHeight, 150)}px`; // Set the new height with a minimum of 150px
        }
    });
